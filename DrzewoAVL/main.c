...
int bfAVL(drzewo **bst)
{
    if ((*bst) == NULL)
        return 0;
    return height(&((*bst)->lwezel)) - height(&((*bst)->pwezel));
}
...
int Add(drzewo **bst, int n)
{
    ...
    else
    {
     ...
     (*bst)->wysokosc = 1 + max(height(&((*bst)->lwezel)), height(&((*bst)->pwezel)));


    int balance = bfAVL(&(*bst));


    if (balance > 1 && n < (*bst)->lwezel->numer)//LL
        return RR(&(*bst));


    if (balance < -1 && n > (*bst)->pwezel->numer)//RR
        return LR(&(*bst));


    if (balance > 1 && n > (*bst)->lwezel->numer)//LR
    {
        (*bst)->lwezel =  LR(&((*bst)->lwezel));
        return RR(&(*bst));
    }


    if (balance < -1 && n < (*bst)->pwezel->numer)//RL
    {
        (*bst)->pwezel = RR(&((*bst)->pwezel));
        return LR(&(*bst));
    }
...
int main()
{
	...
	while(1)
    {
        ...
        printf("[8] Wspolczynnik zbalansowania\n");
        printf("[9] Usun drzewo\n");
        printf("----------------------------------------------\n\n");
		printf("\nWybrana komenda: ");
		scanf("%d", &komenda);
		printf("\n");
